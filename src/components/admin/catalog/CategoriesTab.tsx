...
const EMPTY_FORM: CategoryForm = { name: "", slug: "", parent_id: "", sort_order: "0", active: true };

function toForm(c: AdminCategory): CategoryForm {
  return {
    name: c.name,
    slug: c.slug,
...
// Danh mục cha con tự tham chiếu chính bảng product_categories, không giới
// hạn số cấp — dropdown chọn cha vì vậy phải liệt kê được TOÀN BỘ cây (cấp 1,
// 2, 3, ...), không chỉ cấp 1 như trước (khi đó không thể tạo được cấp 3 vì
// không có cách chọn 1 danh mục cấp 2 làm cha). Khi đang sửa 1 danh mục,
// loại nó và toàn bộ hậu duệ của nó khỏi danh sách để tránh vòng lặp cha-con.
function collectDescendantIds(categories: AdminCategory[], rootId: string): Set<string> {
...
function buildParentOptions(
  ...
  const childrenByParent = new Map<string | null, AdminCategory[]>();
  ...
  return (
    <div className="grid grid-cols-2 gap-2.5">
      <label className="flex flex-col gap-1">
        <span className="text-[10px] font-bold uppercase text-[#94A3B8]">Tên danh mục *</span>
      ...
      <label className="flex flex-col gap-1">
        <span className="text-[10px] font-bold uppercase text-[#94A3B8]">Danh mục cha</span>
        <select
...
export function CategoriesTab() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? "Không tải được danh mục.");
  ...
  return (
    <div>
      <div className="mb-3.5 flex items-center justify-between">
        <span className="text-xs text-[#94A3B8]">{categories.length} danh mục</span>
        <button
          type="button"
              ...
              >
                <div>
                  ...
                  <div className="mt-0.5 text-[11.5px] text-[#64748B]">
                    {c.parent_id ? `Thuộc: ${parentName(c.parent_id) ?? "—"}` : "Danh mục gốc"} · /{c.slug}
                  </div>
                </div>
                <button
                  type="button"
